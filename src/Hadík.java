...
import javax.swing.*;
class Premkiada extends JPanel implements Runnable {
    //
    //
    //By PremyslMikulenka@
    //
    ...
    void drawStartScreen(Graphics2D g) {
        g.setColor(Color.red);
        g.setFont(getFont());
        g.drawString("HADÍK", 240, 190);
        g.setColor(Color.orange);
        g.setFont(smallFont);
        g.drawString("(Klikni K Hraní)", 250, 240);
    }
    ...
    void drawScore(Graphics2D g) {
        ...
        String s = format("Nejvyšší Score: %d    Score: %d", hiScore, score);
        g.drawString(s, 30, h - 30);
        g.drawString(format("Energie: %d", energy), getWidth() - 150, h - 30);
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(
                () -> {
                    ...
                    mainFrame.setTitle("HADÍK");
                    mainFrame.setResizable(true);
                    mainFrame.add(new Premkiada(), BorderLayout.CENTER);
                    mainFrame.pack();
